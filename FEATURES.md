...
- **Per-context unread indicators** - Gold dots show *where* unread activity is: next to "Messages" in the top bar (unread DM notifications) and to the left of each cohort/group in the sidebar (unread posts, comments, mentions, or — for groups — new members). Driven by the `Notification` model and broadcast in real time over the same `[user, :unread_badge]` Turbo stream as the count badges.
